...
for n = 0:0.001:3.14
    ...
    avgang(round(1+1000*n)) = mean(aa(1, :).^2);
end
[~, rotangy] = min(avgang);
rotangy = rotangy/1000;
rotmaty = [cos(rotangy) 0 sin(rotangy); 0 1 0 ; -sin(rotangy) 0 cos(rotangy)];
angles = rotmaty*angles;
% rotation around x-axis
for n = 0:0.001:3.14
    aa = [1 0 0 ; 0 cos(n) -sin(n); 0 sin(n) cos(n)]*angles;
    avgang(round(1+1000*n)) = mean(aa(2, :).^2);
end
[~, rotangx] = min(avgang);
rotangx = rotangx/1000;
rotmatx = [1 0 0 ; 0 cos(rotangx) -sin(rotangx); 0 sin(rotangx) cos(rotangx)];
angles = rotmatx*angles;
G_rot = rotmaty*G;
G_rot = rotmatx*G_rot;


% rotang = zeros(ceil(length(angles)/100), 2);
% for i = 1:ceil(length(angles)/100)
%     anglessub = angles(:, 100*(i-1)+1:min([length(angles) 100*i]));
%     for n = 0:0.01:3.14
%         aa = [cos(n) 0 sin(n); 0 1 0 ; -sin(n) 0 cos(n)]*anglessub;
%         avgang(round(1+100*n)) = mean(aa(1, :).^2);
%     end
%     [~, rotang(i, 1)] = min(avgang);
%     rotang(i, 1) = rotang(i, 1)/100;
%     rotmat = [cos(rotang(i, 1)) 0 sin(rotang(i, 1)); 0 1 0 ; -sin(rotang(i, 1)) 0 cos(rotang(i, 1))];
%     angles(:, 100*(i-1)+1:min([length(angles) 100*i])) = rotmat*anglessub;    
% end
% for i = 1:ceil(length(angles)/100)
%     anglessub = angles(:, 100*(i-1)+1:min([length(angles) 100*i]));
%     for n = 0:0.01:3.14
%         aa = [1 0 0 ; 0 cos(n) -sin(n); 0 sin(n) cos(n)]*anglessub;
%         avgang(round(1+100*n)) = mean(aa(2, :).^2);
%     end
%     [~, rotang(i, 2)] = min(avgang);
%     rotang(i, 2) = rotang(i, 2)/100;
%     rotmat = [1 0 0 ; 0 cos(rotang(i, 2)) -sin(rotang(i, 2)); 0 sin(rotang(i, 2)) cos(rotang(i, 2))];
%     angles(:, 100*(i-1)+1:min([length(angles) 100*i])) = rotmat*anglessub;
% end
...
figure(3); hold on;
plot(timestep(1:length(angles)), angles')
...
% for i = 1:ceil(length(angles)/100)
%     rotmat = [cos(rotang(i, 1)) 0 sin(rotang(i, 1)); 0 1 0 ; -sin(rotang(i, 1)) 0 cos(rotang(i, 1))];
%     Acc(:, 100*(i-1)+1:min([length(angles) 100*i])) = rotmat*Acc(:, 100*(i-1)+1:min([length(angles) 100*i]));
%     rotmat = [1 0 0 ; 0 cos(rotang(i, 2)) -sin(rotang(i, 2)); 0 sin(rotang(i, 2)) cos(rotang(i, 2))];
%     Acc(:, 100*(i-1)+1:min([length(angles) 100*i])) = rotmat*Acc(:, 100*(i-1)+1:min([length(angles) 100*i])); 
% end
...
theta = [0 0 1.5 0 mean(G_rot(3,:)) mean(A_abshorz)]';   % xposition -yposition - abs.speed - heading angle - angular velocity - absolute value
